...
public class Bookstore1Application {
	...
	@Bean
	public CommandLineRunner demo(BookRepository bookRepository, CategoryRepository kRepository) {
		return (args) -> {
		
			log.info("Save a few books"); 
			bookRepository.save(new Book("Vegaanin opas", "Vegaanimies123", 1994, "11111-1", 52));
			bookRepository.save(new Book("Urheilijan venyttelyopas", "Matti Meikäläinen", 2020, "22222-1", 190.20));
			bookRepository.save(new Book("How to make mon€y", "Mukhtar Mahmud", 2001, "10101010-1", 24.95));
			
			
			log.info("Save a few categories");
			kRepository.save(new Category("Kauhu"));
			kRepository.save(new Category("Oppaat")); 
			kRepository.save(new Category("Kaunokirjallisuus")); 
			kRepository.save(new Category("Oppimismateriaali")); 
			kRepository.save(new Category("Mysteerikirjat")); 
